...
model MonoLayerOpaquePCM "Non-fictive single material layer for PCM"

  parameter Modelica.SIunits.Area A "Layer area";
  parameter IDEAS.Buildings.Data.Interfaces.Material mat(PCM=true)
    "Layer material";
  ...
  final parameter Modelica.SIunits.HeatCapacity Cs = (A*mat.rho*mat.cs*mat.d)/nSta;
  final parameter Modelica.SIunits.HeatCapacity Cl = (A*mat.rho*mat.cl*mat.d)/nSta;
  final parameter Modelica.SIunits.Enthalpy Hlat = (A*mat.rho*mat.hlat*mat.d)/nSta;
  Modelica.Blocks.Interfaces.RealOutput E(unit="J") = sum(H);
...
public
  ...
  Modelica.SIunits.Temperature[nSta] T(final start=ones(nSta)*T_start)
    "Temperature at the states";
  Modelica.SIunits.HeatFlowRate[nFlo] Q_flow
    "Heat flow rate from state i to i+1";
    Modelica.SIunits.Enthalpy[nSta] H(start=ones(nSta)*Cs*T_start);
    Real[nSta] fasedebug;
equation
  // connectors
  ...
  for i in 1:nSta loop
    der(H[i]) = (Q_flow[i] - Q_flow[i + 1]);

    if T[i] < mat.T0 then
      H[i]= Cs*T[i];
      fasedebug[i]=1;
    elseif T[i] > mat.T1 then
      H[i]= Cs*mat.T0+(Cs+Cl)/2*(mat.T1-mat.T0)+Hlat+Cl*(T[i]-mat.T1);
      fasedebug[i]=2;
    else
      H[i]= Cs*mat.T0+(Hlat/(mat.T1-mat.T0)+(Cs+Cl)/2)*(T[i]-mat.T0);
      fasedebug[i]=3;
    end if;
 end for;
  annotation (
