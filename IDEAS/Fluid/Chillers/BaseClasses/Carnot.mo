...
partial model Carnot
  ...
  parameter IDEAS.Fluid.Types.EfficiencyInput effInpEva
    ...
    annotation (Dialog(tab="Advanced", group="Temperature dependence"),
                Evaluate=True);
  parameter IDEAS.Fluid.Types.EfficiencyInput effInpCon
  ...
  Real COPCar(min=0) = TUse / IDEAS.Utilities.Math.Functions.smoothMax(
    x1=1,
    x2=TCon-TEva,
    deltaX=0.25) "Carnot efficiency";

  Modelica.SIunits.Temperature TCon(start=TCon_nominal)=
    if effInpCon == IDEAS.Fluid.Types.EfficiencyInput.port_a then
             Medium1.temperature(staA1)
           elseif effInpCon == IDEAS.Fluid.Types.EfficiencyInput.port_b or
                  effInpCon == IDEAS.Fluid.Types.EfficiencyInput.volume then
  ...
  Modelica.SIunits.Temperature TEva(start=TEva_nominal)=
    if effInpEva == IDEAS.Fluid.Types.EfficiencyInput.port_a then
             Medium2.temperature(staA2)
           elseif effInpEva == IDEAS.Fluid.Types.EfficiencyInput.port_b or
                  effInpEva == IDEAS.Fluid.Types.EfficiencyInput.volume then
